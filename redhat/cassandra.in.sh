...
if [ -d $CASSANDRA_HOME/build ] ; then
    jars_cnt="`ls -1 $CASSANDRA_HOME/build/*-[0-9]*.jar 2>/dev/null | grep -v 'javadoc.jar' | grep -v 'sources.jar' | wc -l | xargs echo`"
    ...
    if [ "$jars_cnt" = "1" ]; then
        cassandra_bin="`ls -1 $CASSANDRA_HOME/build/*-[0-9]*.jar 2>/dev/null | grep -v javadoc | grep -v sources`"
