...
public class CompactionBreakdownBench extends BaseCompactionBench
{
    @Benchmark
    ...
    public void iterateThroughPartitionIndexIterator() throws Throwable
    {
        ...
        for (SSTableReader reader : ssTables)
        {
            try (KeyIterator partitionIndexIterator = reader.keyIterator())
            {

                long start = partitionIndexIterator.getBytesRead();
                long end = partitionIndexIterator.getBytesRead();
                while (partitionIndexIterator.hasNext())
                {
                    totalRows++;
                    start = end;
                    partitionIndexIterator.next();
                    end = partitionIndexIterator.getBytesRead();
                }
            }
        }
        if (totalRows != sstableCount * rowsPerSSTable)
    ...
    public void iterateThroughCompactionIterator(Blackhole bh) throws Throwable
    {
        ...
        try (ScannerList scanners = strategy.getScanners(compacting);
             CompactionIterator ci = new CompactionIterator(OperationType.COMPACTION, scanners.scanners, controller, FBUtilities.nowInSeconds(), TimeUUID.Generator.nextTimeUUID()))
        {
    ...
    private long consumeCompactionCursor(CompactionCursor cursor, Blackhole bh, long totalRows) throws IOException
    {
        class Sink implements SSTableDataSink {
            ...
            {
                return Mockito.mock(AbstractRowIndexEntry.class);
            }
