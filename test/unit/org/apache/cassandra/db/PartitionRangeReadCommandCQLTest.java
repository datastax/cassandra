...
import static org.apache.cassandra.cql3.Operator.LIKE;
import static org.apache.cassandra.cql3.Operator.LIKE_CONTAINS;
import static org.apache.cassandra.cql3.Operator.LIKE_MATCHES;
import static org.apache.cassandra.cql3.Operator.LIKE_PREFIX;
import static org.apache.cassandra.cql3.Operator.LIKE_SUFFIX;
...
public class PartitionRangeReadCommandCQLTest extends ReadCommandCQLTester<PartitionRangeReadCommand>
{
    ...
    public void testToCQLString()
    {
        ...
        assertToCQLString("SELECT * FROM %s WHERE v = 1 ALLOW FILTERING", "SELECT * FROM %s WHERE v = 1 ALLOW FILTERING", "SELECT * FROM %s WHERE v = ? ALLOW FILTERING");
        assertToCQLString("SELECT * FROM %s WHERE c = 0 AND v = 1 ALLOW FILTERING", "SELECT * FROM %s WHERE c = 0 AND v = 1 ALLOW FILTERING", "SELECT * FROM %s WHERE c = ? AND v = ? ALLOW FILTERING");
        assertToCQLString("SELECT * FROM %s WHERE c > 0 AND v = 1 ALLOW FILTERING", "SELECT * FROM %s WHERE c > 0 AND v = 1 ALLOW FILTERING", "SELECT * FROM %s WHERE c > ? AND v = ? ALLOW FILTERING");
        assertToCQLString("SELECT * FROM %s WHERE c < 0 AND v = 1 ALLOW FILTERING", "SELECT * FROM %s WHERE c < 0 AND v = 1 ALLOW FILTERING", "SELECT * FROM %s WHERE c < ? AND v = ? ALLOW FILTERING");
        assertToCQLString("SELECT * FROM %s WHERE c >= 0 AND v = 1 ALLOW FILTERING", "SELECT * FROM %s WHERE c >= 0 AND v = 1 ALLOW FILTERING", "SELECT * FROM %s WHERE c >= ? AND v = ? ALLOW FILTERING");
        assertToCQLString("SELECT * FROM %s WHERE c <= 0 AND v = 1 ALLOW FILTERING", "SELECT * FROM %s WHERE c <= 0 AND v = 1 ALLOW FILTERING", "SELECT * FROM %s WHERE c <= ? AND v = ? ALLOW FILTERING");
        ...
        assertToCQLString("SELECT m['key'] FROM %s WHERE m['key'] = 'value' ALLOW FILTERING",
                          ...
                          "SELECT m FROM %s WHERE m[?] = ? ALLOW FILTERING");
        assertToCQLString("SELECT m['key'] FROM %s WHERE m['key'] < 'value' ALLOW FILTERING",
                          "SELECT m FROM %s WHERE m['key'] < 'value' ALLOW FILTERING",
                          "SELECT m FROM %s WHERE m[?] < ? ALLOW FILTERING");
        assertToCQLString("SELECT m['key'] FROM %s WHERE m['key'] > 'value' ALLOW FILTERING",
        ...
        createIndex("CREATE CUSTOM INDEX ON %s(v) USING '" + IndexWithLikeSupport.class.getName() + '\'');
        assertToCQLString("SELECT * FROM %s WHERE v LIKE 'abc'",
                          "SELECT * FROM %s WHERE v LIKE 'abc' ALLOW FILTERING",
                          "SELECT * FROM %s WHERE v LIKE ? ALLOW FILTERING");
        assertToCQLString("SELECT * FROM %s WHERE v LIKE '%%abc'",
                          "SELECT * FROM %s WHERE v LIKE '%%abc' ALLOW FILTERING",
                          "SELECT * FROM %s WHERE v LIKE ? ALLOW FILTERING");
        assertToCQLString("SELECT * FROM %s WHERE v LIKE 'abc%%'",
        ...
        assertToCQLString("SELECT * FROM \"K\".\"T\" WHERE \"V\" = 0 ALLOW FILTERING",
                          ...
                          "SELECT * FROM \"K\".\"T\" WHERE \"V\" = ? ALLOW FILTERING");
        assertToCQLString("SELECT * FROM \"K\".\"T\" WHERE \"S\" = 0 ALLOW FILTERING",
                          "SELECT * FROM \"K\".\"T\" WHERE \"S\" = 0 ALLOW FILTERING",
                          "SELECT * FROM \"K\".\"T\" WHERE \"S\" = ? ALLOW FILTERING");
        assertToCQLString("SELECT * FROM \"K\".\"T\" WHERE \"C\" = 0 ALLOW FILTERING",
                          "SELECT * FROM \"K\".\"T\" WHERE \"C\" = 0 ALLOW FILTERING",
                          "SELECT * FROM \"K\".\"T\" WHERE \"C\" = ? ALLOW FILTERING");

        // The row filter can contain expressions that duplicate the clustering filter restrictions, because indexed
        // clustering key restrictions are added to the row filter at the CQL layer for easier consumption downstream.
        // However, due to CQL semantics the row filter won't contain additional expressions for columns that are
        // included in the clustering filter, and toCQLString methods assume that the internal read commands are built
        // according to that CQL semantics. That CQL validation is tested in Relation tests, but we repeat some of those
        // tests here to detect any changes in CQL that might require changes in the toCQLString() implementation.
        createTable("CREATE TABLE %s (k int, c1 int, c2 int, v int, PRIMARY KEY (k, c1, c2))");
        createIndex("CREATE INDEX ON %s(c1)");
        createIndex("CREATE INDEX ON %s(c2)");
        assertInvalidMessage(format(SingleColumnRestriction.EQRestriction.CANNOT_BE_MERGED_ERROR, "c1"),
                             "SELECT * FROM %s WHERE k = 0 AND c1 = 0 AND c1 = 1 ALLOW FILTERING");
        assertInvalidMessage(format(SingleColumnRestriction.EQRestriction.CANNOT_BE_MERGED_ERROR, "c1"),
                             "SELECT * FROM %s WHERE k = 0 AND c1 = 0 AND c2 = 0 AND c1 = 1 ALLOW FILTERING");
        assertInvalidMessage(format(SingleColumnRestriction.EQRestriction.CANNOT_BE_MERGED_ERROR, "c1"),
                             "SELECT * FROM %s WHERE k = 0 AND c1 = 0 AND c1 IN (1, 2) ALLOW FILTERING");
        assertInvalidMessage(format(SingleColumnRestriction.EQRestriction.CANNOT_BE_MERGED_ERROR, "c1"),
                             "SELECT * FROM %s WHERE k = 0 AND c1 = 0 AND c2 = 0 AND c1 IN (1, 2) ALLOW FILTERING");
        assertInvalidMessage(format(SingleColumnRestriction.INRestriction.CANNOT_BE_MERGED_ERROR, "c1"),
                             "SELECT * FROM %s WHERE k = 0 AND c1 IN (0, 1) AND c1 = 2 ALLOW FILTERING");
        assertInvalidMessage(format(SingleColumnRestriction.INRestriction.CANNOT_BE_MERGED_ERROR, "c1"),
                             "SELECT * FROM %s WHERE k = 0 AND c1 IN (0, 1) AND c2 = 0 AND c1 = 2 ALLOW FILTERING");
        assertInvalidMessage(format(SingleColumnRestriction.INRestriction.CANNOT_BE_MERGED_ERROR, "c1"),
                             "SELECT * FROM %s WHERE k = 0 AND c1 IN (0, 1) AND c2 IN (0, 1) AND c1 = 2 ALLOW FILTERING");
        assertInvalidMessage(format(SingleColumnRestriction.INRestriction.CANNOT_BE_MERGED_ERROR, "c1"),
                             "SELECT * FROM %s WHERE k = 0 AND c1 IN (0, 1) AND c2 IN (0, 1) AND c1 IN (2, 3) ALLOW FILTERING");
        assertInvalidMessage(format(SingleColumnRestriction.EQRestriction.CANNOT_BE_MERGED_ERROR, "c1"),
                             "SELECT * FROM %s WHERE k = 0 AND c1 = 0 AND c1 > 1 ALLOW FILTERING");
        assertInvalidMessage(format(SingleColumnRestriction.EQRestriction.CANNOT_BE_MERGED_ERROR, "c1"),
                             "SELECT * FROM %s WHERE k = 0 AND c1 = 0 AND c2 = 0 AND c1 > 1 ALLOW FILTERING");
    ...
    {
        public IndexWithLikeSupport(ColumnFamilyStore baseCfs, IndexMetadata metadata)
        {
            super(baseCfs, metadata);
        }

        @Override
        public boolean supportsExpression(ColumnMetadata column, Operator op)
        {
            return op == LIKE || op == LIKE_PREFIX || op == LIKE_CONTAINS || op == LIKE_SUFFIX || op == LIKE_MATCHES;
        }
    }
}
