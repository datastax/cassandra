...
public abstract class ReadCommandCQLTester<T extends ReadCommand> extends CQLTester
{
    ...
    {
        assertToCQLString(query, expectedClearCQL, expectedRedactedCQL, null);
    }

    protected void assertToCQLString(String query,
                                     String expectedClearCQL,
                                     ...
                                     @Nullable String expectedErrorMessage)
    {
        ...
        if (expectedErrorMessage == null)
            execute(PATTERN.matcher(actualClearCQL).replaceAll("%%"));
        else
            Assertions.assertThatThrownBy(() -> execute(actualClearCQL)).hasMessageContaining(expectedErrorMessage);
    }
}
