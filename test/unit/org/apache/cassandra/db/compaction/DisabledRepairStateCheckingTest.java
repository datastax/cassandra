    ...
    public void mixedSSTableFailure() throws Exception
    {
        ...
        for (int m = 1; m < 16; ++m) // test all combinations of two or more sstables with different repair marking
        {
            ...
            for (int order = 0; order < Integer.bitCount(m); order++)
            {
                ...
                mutateRepaired(pending1, ActiveRepairService.UNREPAIRED_SSTABLE, TimeUUID.Generator.nextTimeUUID(), false);
                mutateRepaired(pending2, ActiveRepairService.UNREPAIRED_SSTABLE, TimeUUID.Generator.nextTimeUUID(), false);
