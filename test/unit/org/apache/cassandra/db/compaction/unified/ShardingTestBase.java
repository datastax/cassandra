...
public class ShardingTestBase extends CQLTester
{
    ...
    static final int PARTITIONS = 5000;
    ...
    void verifySharding(int numShards, int rowCount, int numOutputSSTables, ColumnFamilyStore cfs) throws Throwable
    {
        ...
        {
            System.out.println("Optimal total BF size: " + FBUtilities.prettyPrintMemory(filter.serializedSize()));
        }
        try (var filter = FilterFactory.getFilter(totalKeyCount / numShards, 0.01))
        {
            System.out.println("Sharded optimal total BF size: " + FBUtilities.prettyPrintMemory(filter.serializedSize() * numShards));
        }
    ...
    {
        return rdr.getBloomFilterSerializedSize();
    }
    ...
    int compact(ColumnFamilyStore cfs, LifecycleTransaction txn, CompactionAwareWriter writer)
    {
        ...
        try (ScannerList scanners = cfs.getCompactionStrategy().getScanners(txn.originals(), ReadCtx.FOR_TEST);
             ...
             CompactionIterator ci = new CompactionIterator(OperationType.COMPACTION, scanners.scanners, controller, nowInSec, UUIDGen.getTimeUUID(System.currentTimeMillis())))
        {
            while (ci.hasNext())
            {
