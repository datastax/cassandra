...
public class ShardingTestBase extends CQLTester
{
    ...
    {
        SSTable.Owner owner = rdr.owner().orElse(null);
        assert owner instanceof ColumnFamilyStore || owner == null;
        ColumnFamilyStore cfs = (ColumnFamilyStore) owner;
        return BloomFilterMetrics.instance.bloomFilterDiskSpaceUsed.getTableGauge(cfs).getValue();
    }
    ...
    int compact(ColumnFamilyStore cfs, LifecycleTransaction txn, CompactionAwareWriter writer)
    {
        ...
        try (ScannerList scanners = cfs.getCompactionStrategy().getScanners(txn.originals());
             ...
             CompactionIterator ci = new CompactionIterator(OperationType.COMPACTION, scanners.scanners, controller, nowInSec,
                                                            TimeUUID.maxAtUnixMillis(System.currentTimeMillis())))
        {
            while (ci.hasNext())
            {
