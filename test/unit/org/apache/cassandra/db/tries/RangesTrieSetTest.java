...
public class RangesTrieSetTest
{
    ...
    private static void verifyTails(ByteComparable[] boundaries, TrieSet set)
    {
        Set<ByteComparable> prefixes = new TreeSet<>(FORWARD_COMPARATOR);
        for (ByteComparable b : boundaries)
        {
            if (b == null)
                continue;
            for (int i = 0; i <= ByteComparable.length(b, VERSION); ++i)
                prefixes.add(ByteComparable.cut(b, i));
        }

        for (ByteComparable prefix : prefixes)
        {
            List<ByteComparable> tails = null;
            ...
            for (Direction dir : Direction.values())
            {
                ...
                assertNotNull(tail);
                dumpToOut(tail);
                var expectations = getExpectations(tails.toArray(ByteComparable[]::new));
    ...
    static NavigableMap<ByteComparable, TrieSetCursor.RangeState> getExpectations(ByteComparable... boundaries)
    {
