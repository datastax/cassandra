...
public class SinglePartitionReadCommandCQLTest extends ReadCommandCQLTester<SinglePartitionReadCommand>
{
    ...
    public void testToCQLString()
    {
        ...
        assertToCQLString("SELECT * FROM %s WHERE k = 0",
                          "SELECT * FROM %s WHERE k = 0 ALLOW FILTERING",
                          "SELECT * FROM %s WHERE k = ? ALLOW FILTERING");

        assertToCQLString("SELECT * FROM %s WHERE k = 0 AND c = 0",
                          "SELECT * FROM %s WHERE k = 0 AND c = 0 ALLOW FILTERING",
        ...
        assertToCQLString("SELECT * FROM %s WHERE k = 0 AND v = 1 ALLOW FILTERING",
                          "SELECT * FROM %s WHERE k = 0 AND v = 1 ALLOW FILTERING",
                          "SELECT * FROM %s WHERE k = ? AND v = ? ALLOW FILTERING");
        assertToCQLString("SELECT * FROM %s WHERE k = 0 AND c = 0 AND v = 1 ALLOW FILTERING",
                          "SELECT * FROM %s WHERE k = 0 AND v = 1 AND c = 0 ALLOW FILTERING",
                          "SELECT * FROM %s WHERE k = ? AND v = ? AND c = ? ALLOW FILTERING");
        assertToCQLString("SELECT * FROM %s WHERE k = 0 AND c > 0 AND v = 1 ALLOW FILTERING",
                          "SELECT * FROM %s WHERE k = 0 AND v = 1 AND c > 0 ALLOW FILTERING",
                          "SELECT * FROM %s WHERE k = ? AND v = ? AND c > ? ALLOW FILTERING");
        assertToCQLString("SELECT * FROM %s WHERE k = 0 AND c < 0 AND v = 1 ALLOW FILTERING",
