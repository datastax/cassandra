...
public class ColumnSelectionTest extends VectorTester
{
    ...
    public void testColumnSelection() throws Throwable
    {
        ...
        Object[] row = row("k1", "k2", "c1", "c2", "s1", "s2", "r1", vector(1.0f, 2.0f));
        ...
        beforeAndAfterFlush(() -> {
            ...
            // ANN ordering
            assertRows(execute("SELECT * FROM %s ORDER BY r2 ANN OF [1.0, 2.0] LIMIT 10"), row);
            assertRows(execute("SELECT k1 FROM %s ORDER BY r2 ANN OF [1.0, 2.0] LIMIT 10"), row(row[0]));
            assertRows(execute("SELECT k2 FROM %s ORDER BY r2 ANN OF [1.0, 2.0] LIMIT 10"), row(row[1]));
            assertRows(execute("SELECT c1 FROM %s ORDER BY r2 ANN OF [1.0, 2.0] LIMIT 10"), row(row[2]));
            assertRows(execute("SELECT c2 FROM %s ORDER BY r2 ANN OF [1.0, 2.0] LIMIT 10"), row(row[3]));
            assertRows(execute("SELECT s1 FROM %s ORDER BY r2 ANN OF [1.0, 2.0] LIMIT 10"), row(row[4]));
            assertRows(execute("SELECT s2 FROM %s ORDER BY r2 ANN OF [1.0, 2.0] LIMIT 10"), row(row[5]));
            assertRows(execute("SELECT r1 FROM %s ORDER BY r2 ANN OF [1.0, 2.0] LIMIT 10"), row(row[6]));
            assertRows(execute("SELECT r2 FROM %s ORDER BY r2 ANN OF [1.0, 2.0] LIMIT 10"), row(row[7]));
            assertRows(execute("SELECT k1, c1, s1, r1 FROM %s ORDER BY r2 ANN OF [1.0, 2.0] LIMIT 10"), row(row[0], row[2], row[4], row[6]));
            assertRows(execute("SELECT k2, c2, s2, r2 FROM %s ORDER BY r2 ANN OF [1.0, 2.0] LIMIT 10"), row(row[1], row[3], row[5], row[7]));
        });
    }
}
