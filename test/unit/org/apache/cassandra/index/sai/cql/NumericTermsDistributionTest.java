...
public class NumericTermsDistributionTest extends SAITester
{
    ...
    public void testNumericIndexEstimates() throws Throwable
    {
        ...
        assertInMemoryEstimateCount(sai, Operator.LT, "0", 2, 1);
        assertInMemoryEstimateCount(sai, Operator.GT, "0", 3, 1);
        ...
        assertSSTableEstimateCount(sai, Operator.LT, "0", 2, 1);
        assertSSTableEstimateCount(sai, Operator.GT, "0", 3, 1);
    ...
    private void assertInMemoryEstimateCount(StorageAttachedIndex index, Operator op, String value, long expectedCount, long roundingValue)
    {
        ...
        if (TypeUtil.supportsRounding(testType.getType()))
            assertEquals(expectedCount + roundingValue, memoryCount);
        else
            assertEquals(expectedCount, memoryCount);
    }
    ...
    private void assertSSTableEstimateCount(StorageAttachedIndex index, Operator op, String value, long expectedCount, long roundingValue)
    {
        var expression = buildExpression(index, op, value);
        var wholeRange = DataRange.allData(index.getIndexContext().getPartitioner()).keyRange();
        ...
        // it is automatically inclusive to prevent missed results.
        if (TypeUtil.supportsRounding(testType.getType()))
            assertEquals(expectedCount + roundingValue, onDiskCount);
        else
            assertEquals(expectedCount, onDiskCount);
    }
