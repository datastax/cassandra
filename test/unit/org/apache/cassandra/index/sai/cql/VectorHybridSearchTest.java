...
public class VectorHybridSearchTest extends VectorTester.VersionedWithChecksums
{
    ...
    public void testReranklessHybridSearch()
    {
        ...
        createTable("CREATE TABLE %s (pk int, val int, vec vector<float, 10>, PRIMARY KEY(pk))");
        createIndex("CREATE CUSTOM INDEX ON %s(vec) USING 'StorageAttachedIndex'");
        createIndex("CREATE CUSTOM INDEX ON %s(val) USING 'StorageAttachedIndex'");

        // Insert many rows in parallel
        IntStream.range(0, MIN_PQ_ROWS * 2).parallel().forEach(i -> {
            execute("INSERT INTO %s (pk, val, vec) VALUES (?, ?, ?)", i, i, randomVectorBoxed(10));
        });
        ...
        setMaxBruteForceRows(0);
        var result = execute("SELECT pk FROM %s WHERE val < ? ORDER BY vec ANN OF ? LIMIT 10 with ann_options = { 'rerank_k': 0 }", MIN_PQ_ROWS / 2, randomVectorBoxed(10));
        ...
        result = execute("SELECT pk FROM %s WHERE val < ? ORDER BY vec ANN OF ? LIMIT 10 with ann_options = { 'rerank_k': 0 }", MIN_PQ_ROWS / 5, randomVectorBoxed(10));
        assertRowCount(result, 10);
        // Also ensure that a negative rerank_k value works
        result = execute("SELECT pk FROM %s WHERE val < ? ORDER BY vec ANN OF ? LIMIT 10 with ann_options = { 'rerank_k': -1 }", MIN_PQ_ROWS / 5, randomVectorBoxed(10));
        assertRowCount(result, 10);
    }
}
