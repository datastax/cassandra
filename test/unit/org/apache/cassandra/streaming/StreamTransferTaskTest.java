...
public class StreamTransferTaskTest
{
    ...
    public void testFailSessionDuringTransferShouldNotReleaseReferences() throws Exception
    {
        ...
        // create streaming task that streams those two sstables
        StreamTransferTask task = new StreamTransferTask(session, cfs.metadata.id);
        List<Ref<? extends AbstractSSTableReader>> refs = new ArrayList<>(cfs.getLiveSSTables().size());
        for (AbstractSSTableReader sstable : cfs.getLiveSSTables())
        {
            List<Range<Token>> ranges = new ArrayList<>();
            ranges.add(new Range<>(sstable.first.getToken(), sstable.last.getToken()));
            Ref<? extends AbstractSSTableReader> ref = sstable.selfRef();
            refs.add(ref);
        ...
        for (Ref<? extends AbstractSSTableReader> ref : refs)
        {
