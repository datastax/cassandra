...
public class MemtableAllocatorAwaitRoomTest
{
    ...
    public void zeroLimitPoolDoesNotBlock() throws Exception
    {
        ...
        SlabPool pool = new SlabPool(1 << 20, 0, 1.0f, () -> CompletableFuture.completedFuture(false));
        try
        {
            MemtableAllocator allocator = pool.newAllocator();
    ...
    {
        SlabPool pool = new SlabPool(1 << 20, 1 << 20, 1.0f, () -> CompletableFuture.completedFuture(false));
        try
        {
            MemtableAllocator allocator = pool.newAllocator();
            OpOrder.Group g = new OpOrder().start();
