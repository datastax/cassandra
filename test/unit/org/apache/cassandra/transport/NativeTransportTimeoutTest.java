...
public class NativeTransportTimeoutTest extends CQLTester
{
    ...
    private void doTestLoadShedding(boolean useAsyncStages, Statement statement) throws InterruptedException
    {
        ...
        Thread.sleep(10 + TimeUnit.MILLISECONDS.convert(MonotonicClock.Global.approxTime.error(), TimeUnit.NANOSECONDS) * 2);
        ...
        double queueTimer_in_micros = queueTimer.getSnapshot().get999thPercentile();
        long queuetimer_in_nanos = TimeUnit.NANOSECONDS.convert((long) queueTimer_in_micros, TimeUnit.MICROSECONDS);
        Assert.assertTrue(queuetimer_in_nanos > TimeUnit.NANOSECONDS.convert(10, TimeUnit.MILLISECONDS));
    }
}
