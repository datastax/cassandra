...
public class ReadCallbackTest extends AbstractReadResponseTest
{
    ...
    @Test
    public void testTimeoutLogging() throws Throwable {
            // Set up logging capture for ReadCallback
            ...
            try
            {
                ...
                {
                    System.out.println("=== Captured Log Messages ===");
                    for (ILoggingEvent event : listAppender.list)
                    {
                        System.out.println(event.getLevel() + ": " + event.getFormattedMessage());
                    }
                    System.out.println("=== End Log Messages ===");
    ...
    @Test
    public void testFailureLoggingWithReplicaDetails() throws Throwable
    {
        Logger readCallbackLogger = (Logger) LoggerFactory.getLogger(ReadCallback.class);
        ListAppender<ILoggingEvent> listAppender = new ListAppender<>();
        listAppender.start();
        ...
        try
        {
            ...
            {
                boolean foundFailureLog = listAppender.list.stream()
                                                           .anyMatch(event ->
                                                                     event.getFormattedMessage().contains("Failure:") &&
                                                                     event.getFormattedMessage().contains("replica=") &&
                                                                     event.getFormattedMessage().contains("reason=") &&
                                                                     event.getFormattedMessage().contains("elapsed=")
                                                           );
                assertTrue("Should log failure details with replica and reason", foundFailureLog);
                boolean foundReplicaAddress = listAppender.list.stream()
                                                               .anyMatch(event -> event.getFormattedMessage().contains(EP1.getHostAddress(true)));
                assertTrue("Should log replica address in failure message", foundReplicaAddress);
                boolean foundFailureReason = listAppender.list.stream()
                                                              .anyMatch(event -> event.getFormattedMessage().contains("TIMEOUT"));
                assertTrue("Should log failure reason in message", foundFailureReason);
                boolean foundTimeMetrics = listAppender.list.stream()
                                                            .anyMatch(event -> {
                                                                String msg = event.getFormattedMessage();
                                                                return msg.contains("timeout=") &&
                                                                       msg.contains("available=");
                                                            });
                assertTrue("Should log timeout and available time metrics", foundTimeMetrics);
            }
            finally
            {
                Tracing.instance.stopSession();
