...
public class ReadExecutorTest
{
    ...
    public void testUnableToSpeculate() throws Throwable
    {
        ...
        AbstractReadExecutor executor = new AbstractReadExecutor.NeverSpeculatingReadExecutor(cfs, new MockSinglePartitionReadCommand(), plan(targets, LOCAL_QUORUM), Dispatcher.RequestTime.forImmediateExecution(), true, noopReadTracker());
        ...
        //Shouldn't increment
        executor = new AbstractReadExecutor.NeverSpeculatingReadExecutor(cfs, new MockSinglePartitionReadCommand(), plan(targets, LOCAL_QUORUM), Dispatcher.RequestTime.forImmediateExecution(), false, noopReadTracker());
        executor.maybeTryAdditionalReplicas();
        try
        {
            executor.awaitResponses();
    ...
    public void testSpeculateSucceeded() throws Throwable
    {
        ...
        AbstractReadExecutor executor = new AbstractReadExecutor.SpeculatingReadExecutor(cfs, new MockSinglePartitionReadCommand(DAYS.toMillis(365)), plan(LOCAL_QUORUM, targets, targets.subList(0, 2)), Dispatcher.RequestTime.forImmediateExecution(), noopReadTracker());
        executor.maybeTryAdditionalReplicas();
        new Thread()
        {
            @Override
    ...
    public void testSpeculateFailed() throws Throwable
    {
        ...
        assertEquals(0, ks.metric.speculativeRetries.getCount());
        assertEquals(0, ks.metric.speculativeFailedRetries.getCount());
        AbstractReadExecutor executor = new AbstractReadExecutor.SpeculatingReadExecutor(cfs, new MockSinglePartitionReadCommand(), plan(LOCAL_QUORUM, targets, targets.subList(0, 2)), Dispatcher.RequestTime.forImmediateExecution(), noopReadTracker());
        executor.maybeTryAdditionalReplicas();
        try
        {
            executor.awaitResponses();
    ...
    public void testRaceWithNonSpeculativeFailure()
    {
        ...
        AbstractReadExecutor executor = new AbstractReadExecutor.SpeculatingReadExecutor(cfs, command, plan, Dispatcher.RequestTime.forImmediateExecution(), noopReadTracker());
