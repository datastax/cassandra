...
public class RangeCommandIteratorTest
{
    ...
    public void testRangeCountWithRangeMerge()
    {
        ...
        for (int i = 0; i + 1 < tokens.size(); i++)
        {
            ...
            ranges.add(ReplicaPlans.forRangeRead(keyspace, null, ConsistencyLevel.ONE, range, 1, false));
            vnodeCount++;
        }
    ...
    {
        CloseableIterator<ReplicaPlan.ForRangeRead> replicaPlans = new ReplicaPlanIterator(keyRange, null, keyspace, ConsistencyLevel.ONE, false);
        if (withRangeMerger)
