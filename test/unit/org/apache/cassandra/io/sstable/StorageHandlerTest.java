...
public class StorageHandlerTest
{
    ...
    public void testOnOpeningWrittenSSTableFailure()
    {
        ...
        onOpeningWrittenSSTableFailureInterceptor = (reason, descriptor, components, compressedSize, uncompressedSize, stats, firstKey, lastKey, throwable) -> {
    ...
    {
        private TestStorageHandler(TableMetadataRef metadata, Directories directories, Tracker dataTracker)
        {
        ...
        public SSTableReader onOpeningWrittenSSTableFailure(SSTableReader.OpenReason reason, Descriptor descriptor, Set<Component> components, long compressedSize, long uncompressedSize, StatsMetadata stats, DecoratedKey firstKey, DecoratedKey lastKey, Throwable throwable)
        {
            if (onOpeningWrittenSSTableFailureInterceptor != null)
            {
                try
                {
                    onOpeningWrittenSSTableFailureInterceptor.call(reason, descriptor, components, compressedSize, uncompressedSize, stats, firstKey, lastKey, throwable);
                }
                catch (Exception e)
                {
            ...
            return super.onOpeningWrittenSSTableFailure(reason, descriptor, components, compressedSize, uncompressedSize, stats, firstKey, lastKey, throwable);
        }
    }
    ...
    {
        void call(SSTableReader.OpenReason reason, Descriptor descriptor, Set<Component> components, long compressedSize, long uncompressedSize, StatsMetadata stats, DecoratedKey firstKey, DecoratedKey lastKey, Throwable throwable) throws Exception;
    }

    public static class TestStorageHandlerFactory implements StorageHandlerFactory
    {
        @Override
        public StorageHandler create(TableMetadataRef metadata, Directories directories, Tracker dataTracker)
        {
            return new TestStorageHandler(metadata, directories, dataTracker);
        }
    }
}