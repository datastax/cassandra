...
public class CompactionControllerConfigTest extends TestBaseImpl
{
    ...
    {
        testOrphanedControllerConfigFileCleanup(cluster -> cluster.schemaChange("DROP TABLE test_ks.test_table;"));
    }
    ...
    {
        testOrphanedControllerConfigFileCleanup(cluster -> cluster.schemaChange("DROP KEYSPACE test_ks;"));
    }
    ...
    private void testOrphanedControllerConfigFileCleanup(Consumer<Cluster> schemaRemover) throws Throwable
    {
        try (Cluster cluster = init(Cluster.build(1).start()))
        {
            ...
            cluster.get(1).forceCompact("test_ks", "test_table");
            ...
            cluster.get(1).runOnInstance(() -> {
                // we are calling storeControllerConfig by hand as it's first call might be delayed
                CompactionManager.storeControllerConfig();
                TableMetadata metadata = TableMetadata.minimal("test_ks", "test_table");
