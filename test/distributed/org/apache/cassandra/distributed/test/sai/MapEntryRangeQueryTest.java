...
public class MapEntryRangeQueryTest extends TestBaseImpl
{
    ...
    public void testRangeQueryOnMapEntries()
    {
        ...
        assertThat(Arrays.asList(execute("SELECT pk FROM %s WHERE inventory['apple'] >= 50")))
        .hasSameElementsAs(getExpectedFilteredResults(values, i -> i >= 50));

        assertThat(Arrays.asList(execute("SELECT pk FROM %s WHERE inventory['apple'] > 25")))
        .hasSameElementsAs(getExpectedFilteredResults(values, i -> i > 25));

        assertThat(Arrays.asList(execute("SELECT pk FROM %s WHERE inventory['apple'] <= 30")))
        .hasSameElementsAs(getExpectedFilteredResults(values, i -> i <= 30));

        assertThat(Arrays.asList(execute("SELECT pk FROM %s WHERE inventory['apple'] < 56")))
