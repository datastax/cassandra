...
public class SecondaryIndexTest extends TestBaseImpl
{
    ...
    public void test_secondary_rebuild_with_small_memtable_memory()
    {
        ...
        Object originalMemTableHeapSpace = cluster.get(1).config().get("memtable_heap_space_in_mb");
        cluster.get(1).config().set("memtable_heap_space_in_mb", "1");
        ...
        {
            // restore node1 to use default value for memtable_heap_space_in_mb
            waitOn(cluster.get(1).shutdown());
            cluster.get(1).config().set("memtable_heap_space_in_mb", originalMemTableHeapSpace);
            cluster.get(1).startup();
        }
    }
}