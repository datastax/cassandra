...
public class CoordinatorSensorRegistrySyncTest extends TestBaseImpl
{
    ...
    public void testWriteSensorsSyncedToRegistry()
    {
        ...
        (IIsolatedExecutor.SerializableConsumer<AtomicReference<double[]>>) r -> {
            ...
            r.set(new double[]{ registryValueOnNode(tbl, Type.WRITE_BYTES),
                                registryValueOnNode(null, Type.WRITE_COST) });
        ...
        (IIsolatedExecutor.SerializableConsumer<AtomicReference<double[]>>) r -> r.set(new double[]{ registryValueOnNode(tbl, Type.WRITE_BYTES),
                                                                                                     registryValueOnNode(null, Type.WRITE_COST) })).accept(replicaRef);
    ...
    /**
     * Returns the registry value of the given {@code type} on the calling node.
     * <p>If {@code table} is {@code null}, looks up the sensor on the request-level
     * {@link Context#request()} context (used for {@link Type#READ_COST},
     * {@link Type#WRITE_COST}, and {@link Type#TOTAL_COST}).
     * Otherwise filters by table name; each test uses a dedicated table so there is at most
     * one sensor per (table, type) pair.
     * Must be {@code static} to be serializable across the in-process classloader boundary.
     */
    private static double registryValueOnNode(@Nullable String table, Type type)
    {
        if (table == null)
            return SensorsRegistry.instance
                   .getSensor(Context.from(new org.apache.cassandra.sensors.ActiveRequestSensors()), type)
                   .map(org.apache.cassandra.sensors.Sensor::getValue)
                   .orElse(0.0);
