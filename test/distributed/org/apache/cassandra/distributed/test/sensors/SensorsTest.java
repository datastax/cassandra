...
public class SensorsTest extends TestBaseImpl
{
    ...
    public void testSensorsInResultMessage() throws Throwable
    {
        ...
        try (Cluster cluster = init(Cluster.build(NODES_COUNT).start()))
        {
            ...
            for (String prepQuery : prepQueries)
                cluster.coordinator(1).execute(withKeyspace(prepQuery), ConsistencyLevel.ALL);
            cluster.coordinator(1).execute(withKeyspace("INSERT INTO %s.tbl(pk, v1) VALUES (1, 'read me')"), ConsistencyLevel.ALL);
            // work around serializability of @Parameterized.Parameter by providing locally scoped variables
            String query = testQuery;
            String[] headers = this.expectedHeaders;
            ...
            cluster.get(1).runOnInstance(() -> {
                ResultMessage<?> result = executeWithResult(query);
                Map<String, ByteBuffer> customPayload = result.getCustomPayload();
