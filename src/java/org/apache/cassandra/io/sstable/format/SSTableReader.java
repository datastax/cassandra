...
public abstract class SSTableReader extends SSTable implements SelfRefCounted<SSTableReader>, CompactionSSTable
{
     ...
     * underlying sstable. And suppose that `getPositionsForRanges([[t_s, t_e)])` returns `(k_s, k_e)` (by we mean,
     * the position to key `k_s` (for some value `s`) and the position to key `k_e` (for some value `e` > `s`). Then
     * this method applied to this same range may return either one of `(k_s, k_e)` (same result), `(k_s-1, k_e)`,
     * `(k_s, k_e+1)`, or `(k_s-1, k_e+1)`
     * <p>
     * Also note that as a consequence of this, the returned list of position bounds may have some strict overlap
     * (the method could return something along the lines of `[(0, 100), (80, 200)]`)
     */
