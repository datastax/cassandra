        ...
        org.apache.hadoop.mapred.RecordWriter<Map<String, ByteBuffer>, List<ByteBuffer>>, AutoCloseable
{
    ...
    CqlRecordWriter(Configuration conf)
    {
        ...
        try
        {
            ...
            if (session != null)
            {
                TableMetadata tableMetadata = session.getCluster().getMetadata().getKeyspace(session.getLoggedKeyspace()).getTable(ConfigHelper.getOutputColumnFamily(conf));
                ...
                while (run || !queue.isEmpty())
                {
                    ...
                    while (true)
                    {
                        ...
                        try
                        {
                            ...
                            cluster = CqlConfigHelper.getOutputCluster(host, conf, nettyOptions);
