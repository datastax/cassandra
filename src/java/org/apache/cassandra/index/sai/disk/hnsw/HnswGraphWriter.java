...
public class HnswGraphWriter
{
    ...
    private long levelSize(int level) throws IOException
    {
        ...
        HnswGraph.NodesIterator nodesOnLevel = hnsw.getNodesOnLevel(level);
    ...
    private long neighborSize(int level, int node) throws IOException
    {
        ...
        int n = hnsw.getNeighborCount(level, node);
        return 4L * (1 + n);
    }

    public long write(IndexOutputWriter indexOutputWriter) throws IOException
    {
        SequentialWriter out = indexOutputWriter.asSequentialWriter();
        ...
        HashMap<Integer, Long> levelOffsets = new HashMap<Integer, Long>(); // VSTODO remove this once the code is debugged
        ...
        for (int level = 0; level < hnsw.numLevels(); level++)
        {
            ...
            int[] sortedNodes = ExtendedHnswGraph.getSortedNodes(hnsw, level);
            ...
            HashMap<Integer, Long> nodeOffsets = new HashMap<Integer, Long>(); // VSTODO remove this once the code is debugged
