 ...
 */
package org.apache.cassandra.index.sai.disk.v1dse.kdtree;
...
import org.apache.cassandra.index.sai.disk.v1dse.DirectReaders;
import org.apache.cassandra.index.sai.disk.v1dse.io.CryptoUtils;
import org.apache.cassandra.index.sai.disk.v1dse.io.IndexFileUtils;
import org.apache.cassandra.index.sai.disk.v1dse.io.SeekingRandomAccessInput;
import org.apache.cassandra.index.sai.disk.v1dse.lucene75.index.CorruptIndexException;
import org.apache.cassandra.index.sai.disk.v1dse.lucene75.store.IndexInput;
import org.apache.cassandra.index.sai.disk.v1dse.lucene75.util.BytesRef;
import org.apache.cassandra.index.sai.disk.v1dse.lucene75.util.FixedBitSet;
import org.apache.cassandra.index.sai.disk.v1dse.lucene75.util.FutureArrays;
import org.apache.cassandra.index.sai.disk.v1dse.lucene75.util.packed.DirectWriter;
import org.apache.cassandra.index.sai.disk.v1dse.postings.FilteringPostingList;
import org.apache.cassandra.index.sai.disk.v1dse.postings.MergePostingList;
import org.apache.cassandra.index.sai.disk.v1dse.postings.PostingsReader;
...
public class BKDReader extends TraversingBKDReader implements Closeable
{
    ...
    public class IteratorState extends AbstractGuavaIterator<Long> implements Comparable<IteratorState>, Closeable
    {
                        ...
                        LongArrayList tempPostings) throws IOException
    {
        ...
        final IntersectVisitor visitor = new IntersectVisitor() {
            ...
            @Override
            public BKDQueries.Relation compare(byte[] minPackedValue, byte[] maxPackedValue) {
                return BKDQueries.Relation.CELL_CROSSES_QUERY;
            }
        };
    ...
    {
        BKDQueries.Relation relation = visitor.compare(minPackedValue, maxPackedValue);

        if (relation == BKDQueries.Relation.CELL_OUTSIDE_QUERY)
        {
        ...
        relation == BKDQueries.Relation.CELL_INSIDE_QUERY ?
    ...
    class Intersection
    {
        ...
        Intersection(IndexInput bkdInput, IndexInput postingsInput, IndexInput postingsSummaryInput,
                     IndexTree index, QueryEventListener.BalancedTreeEventListener listener, QueryContext context)
        {
        ...
        public void collectPostingLists(PriorityQueue<PeekablePostingList> postingLists) throws IOException
        {
            context.checkpoint();
            ...
            if (postingsIndex.exists(nodeID))
            {
                postingLists.add(PeekablePostingList.makePeekable(initPostingReader(postingsIndex.getPostingsFilePointer(nodeID))));
                return;
            }
...
//                System.out.println("dim * bytesPerDim="+(dim * bytesPerDim)+" prefix="+prefix+" numDims="+numDims);
    ...
    private class FilteringIntersection extends Intersection
    {
        ...
        public void collectPostingLists(PriorityQueue<PeekablePostingList> postingLists, byte[] cellMinPacked, byte[] cellMaxPacked) throws IOException
        {
            context.checkpoint();

            final BKDQueries.Relation r = visitor.compare(cellMinPacked, cellMaxPacked);

            if (r == BKDQueries.Relation.CELL_OUTSIDE_QUERY)
            {
                // This cell is fully outside of the query shape: stop recursing
                return;
            }

            if (r == BKDQueries.Relation.CELL_INSIDE_QUERY)
            {
        ...
        @SuppressWarnings("resource")
        void filterLeaf(PriorityQueue<PeekablePostingList> postingLists) throws IOException
        {
            ...
            {
                final long pointer = postingsIndex.getPostingsFilePointer(nodeID);
                postingLists.add(PeekablePostingList.makePeekable(initFilteringPostingReader(pointer, holder[0])));
            }
        }
    ...
    public interface IntersectVisitor
    {
         ...
         */
        BKDQueries.Relation compare(byte[] minPackedValue, byte[] maxPackedValue);
    }
}
