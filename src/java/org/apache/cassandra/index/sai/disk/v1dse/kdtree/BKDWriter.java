...
import org.apache.cassandra.index.sai.disk.v1dse.SAICodecUtils;
import org.apache.cassandra.index.sai.disk.v1dse.io.CryptoUtils;
import org.apache.cassandra.index.sai.disk.v1dse.lucene75.store.DataOutput;
import org.apache.cassandra.index.sai.disk.v1dse.lucene75.store.GrowableByteArrayDataOutput;
import org.apache.cassandra.index.sai.disk.v1dse.lucene75.store.IndexOutput;
import org.apache.cassandra.index.sai.disk.v1dse.lucene75.util.ArrayUtil;
import org.apache.cassandra.index.sai.disk.v1dse.lucene75.util.BytesRef;
import org.apache.cassandra.index.sai.disk.v1dse.lucene75.util.BytesRefBuilder;
import org.apache.cassandra.index.sai.disk.v1dse.lucene75.util.FutureArrays;
import org.apache.cassandra.index.sai.disk.v1dse.lucene75.util.IntroSorter;
import org.apache.cassandra.index.sai.disk.v1dse.lucene75.util.Sorter;
...
public class BKDWriter implements Closeable
{
    ...
    /**
     * Write a field from a {@link BlockBalancedTreeIterator}. This way of writing
     * points is faster than regular writes with BKDWriter#add since
     * there is opportunity for reordering points before writing them to
     * disk. This method does not use transient disk in order to reorder points.
     */
    public long writeField(IndexOutput out, BlockBalancedTreeIterator values, Callback callback) throws IOException
    {
        SAICodecUtils.writeHeader(out);

        final LeafWriter leafWriter = new LeafWriter(out, callback);
        ...
        long treeFilePointer = leafWriter.finish();
    ...
    private class LeafWriter
    {
        ...
        LeafWriter(IndexOutput out, Callback callback)
        {
            if (numDims != 1)
            {
  ...
  */
    ...
    private int appendBlock(ResettableByteBuffersIndexOutput writeBuffer, List<byte[]> blocks) throws IOException
    {
        ...
        writeBuffer.writeBytes(bytes, 0, bytes.length);
        writeBuffer.reset();
        blocks.add(bytes);
        return pos;
    }
                                 ...
                                 int nodeID, byte[] lastSplitValues, boolean[] negativeDeltas, boolean isLeft) throws IOException
    {
        ...
        else
        {
            ...
            writeBuffer.writeBytes(bytes2, 0, bytes2.length);
            writeBuffer.reset();
    ...
    private void writeIndex(IndexOutput out, int countPerLeaf, int numLeaves, byte[] packedIndex) throws IOException
    {
        ...
        if (compressor != null)
        {
            try (ResettableByteBuffersIndexOutput writeBuffer = new ResettableByteBuffersIndexOutput("PackedIndex"))
            {
                writeBuffer.writeBytes(minPackedValue, 0, packedBytesLength);
                writeBuffer.writeBytes(maxPackedValue, 0, packedBytesLength);

                CryptoUtils.compress(new BytesRef(writeBuffer.toArrayCopy(), 0, (int) writeBuffer.getFilePointer()), out, compressor);
            }
        }
        else
        {
