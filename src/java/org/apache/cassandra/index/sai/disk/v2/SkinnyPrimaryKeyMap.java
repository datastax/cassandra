...
public class SkinnyPrimaryKeyMap implements PrimaryKeyMap
{
    ...
    public long rowIdFromPrimaryKey(PrimaryKey key)
    {
        ...
        if (key.isTokenOnly() || rowId < 0 || rowId + 1 == tokenArray.length() || tokenArray.get(rowId) != key.token().getLongValue())
            return rowId;
        // The first index might not have been the correct match in the case of token collisions.
        return tokenCollisionDetection(key, rowId);
    }
...
//        }
//
//        return partitionKeyCursor.ceiling(key::asComparableBytesMinPrefix);
//    }
