...
public class V2VectorIndexSearcher extends IndexSearcher implements SegmentOrdering
{
    ...
    private int topKFor(int limit)
    {
        ...
        double n = 0.509 + 9.491 * pow(limit, -0.402); // f(1) = 10.0, f(100) = 2.0, f(1000) = 1.1
    ...
    private void updateExpectedNodes(int actualNodesVisited, int rawExpectedNodes)
    {
        ...
        int expectedNodes = (int) (ratio * rawExpectedNodes);
    ...
    public RangeIterator limitToTopResults(QueryContext context, List<PrimaryKey> keys, Expression exp, int limit) throws IOException
    {
             ...
             OrdinalsView ordinalsView = graph.getOrdinalsView())
        {
            ...
            Histogram comparisonsSavedByBsearch = new Histogram(new SlidingWindowReservoir(10));
            ...
            for (int i = 0; i < keysInRange.size(); i++)
            {
                ...
                PrimaryKeyWithSource pkws = (PrimaryKeyWithSource) primaryKey;
                long sstableRowId;
                ...
                if (sstableRowId < 0)
                {
                    ...
                    else
                    {
                        ...
                        List<PrimaryKey> keysRemaining = keysInRange.subList(i, keysInRange.size());
