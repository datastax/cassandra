...
public class QueryController
{
    ...
    public RangeIterator<PrimaryKey> getIndexes(Operation.OperationType op, Collection<Expression> expressions)
    {
        ...
        Expression annExpressionInHybridSearch = getAnnExpressionInHybridSearch(expressions);
        ...
        QueryViewBuilder.QueryView annQueryViewInHybridSearch = isAnnHybridSearch ? new QueryViewBuilder(Collections.singleton(annExpressionInHybridSearch), mergeRange).build() : null;
        ...
        try
        {
            List<RangeIterator<PrimaryKey>> sstableIntersections = queryView.view.entrySet()
                                                                                 ...
                                                                                 .map(e -> {
                                                                                     ...
                                                                                     PrimaryKeyMap.Factory pkFactory = e.getValue().iterator().next().index.getSSTableContext().primaryKeyMapFactory;
    ...
    {
        List<RangeIterator<Long>> subIterators = indexExpressions
                           .stream()
                           .map(ie ->
                                    {
                                        try
