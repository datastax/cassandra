...
public class StorageService extends NotificationBroadcasterSupport implements IEndpointStateChangeSubscriber, StorageServiceMBean
{
    ...
    {
        initServer(SCHEMA_DELAY_MILLIS);
    }

    public synchronized void initServer(int schemaTimeoutMillis) throws ConfigurationException
    {
        ...
        if (joinRing)
        {
            joinTokenRing(schemaTimeoutMillis);
        }
        else
        {
            Collection<Token> tokens = SystemKeyspace.getSavedTokens();
    ...
    private void prepareToJoin() throws ConfigurationException
    {
        ...
        if (!joined)
        {
            ...
            Gossiper.instance.addLocalApplicationState(ApplicationState.SCHEMA, StorageService.instance.valueFactory.schema(SchemaManager.instance.schema().getVersion()));
    ...
    public void waitForSchema(long timeoutMillis)
    {
        if (!SchemaManager.instance.waitUntilReady(Duration.ofMillis(timeoutMillis)))
            throw new IllegalStateException("Could not achieve schema readiness in " + Duration.ofMillis(timeoutMillis));
    }
    ...
    private void joinTokenRing(boolean finishJoiningRing,
    ...
    private void executePreJoinTasks(boolean bootstrap)
    {
        StreamSupport.stream(ColumnFamilyStore.all().spliterator(), false)
                .filter(cfs -> SchemaManager.instance.getUserKeyspaces().names().contains(cfs.keyspace.getName()))
    ...
    private void doAuthSetup(boolean setUpSchema)
    {
        if (!authSetupCalled.getAndSet(true))
        {
            if (setUpSchema)
            {
                SchemaManager.instance.apply(SchemaTransformations.updateSystemKeyspace(AuthKeyspace.metadata(), AuthKeyspace.GENERATION), true);
            }
    ...
    {
        SchemaManager.instance.apply(SchemaTransformations.updateSystemKeyspace(TraceKeyspace.metadata(), TraceKeyspace.GENERATION), true);
        SchemaManager.instance.apply(SchemaTransformations.updateSystemKeyspace(SystemDistributedKeyspace.metadata(), SystemDistributedKeyspace.GENERATION), true);
        SchemaManager.instance.apply(SchemaTransformations.updateSystemKeyspace(AuthKeyspace.metadata(), AuthKeyspace.GENERATION), true);
    }
    ...
    @VisibleForTesting
    public Collection<InetAddressAndPort> prepareForBootstrap(long schemaTimeoutMillis)
    {
        ...
        waitForSchema(schemaTimeoutMillis);
        ...
        else
        {
            if (!isReplacingSameAddress())
            {
                ...
                for (Token token : bootstrapTokens)
                {
                    ...
                    if (existing != null)
                    {
                        long nanoDelay = schemaTimeoutMillis * 1000000L;
    ...
    private void markViewsAsBuilt() {
        for (String keyspace : SchemaManager.instance.getUserKeyspaces().names())
        {
    ...
    private Map<List<String>, List<String>> getPendingRangeToEndpointMap(String keyspace, boolean withPort)
    {
        ...
        // non-system keyspace.
        if (keyspace == null)
            keyspace = SchemaManager.instance.getNonLocalStrategyKeyspaces().iterator().next().name;
    ...
    {
        if (!SchemaManager.instance.getKeyspaces().contains(keyspace))
    ...
    */
    ...
    private void verifyKeyspaceIsValid(String keyspaceName)
    {
        ...
        if (!SchemaManager.instance.getKeyspaces().contains(keyspaceName))
    ...
    */
    ...
    public void decommission(boolean force) throws InterruptedException
    {
        ...
        try
        {
            ...
            {
                int rf, numNodes;
                for (String keyspaceName : SchemaManager.instance.getPartitionedKeyspaces().names())
                {
                    if (!force)
                    {
    ...
    private void move(Token newToken) throws IOException
    {
        ...
        List<String> keyspacesToProcess = ImmutableList.copyOf(SchemaManager.instance.getNonLocalStrategyKeyspaces().names());
    ...
    private LinkedHashMap<InetAddressAndPort, Float> getEffectiveOwnership(String keyspace)
    {
        ...
        {
            Collection<String> userKeyspaces = SchemaManager.instance.getUserKeyspaces().names();
    ...
    {
        List<String> keyspaceNamesList = new ArrayList<>(SchemaManager.instance.getKeyspaces());
        return Collections.unmodifiableList(keyspaceNamesList);
    }

    public List<String> getNonSystemKeyspaces()
    {
        List<String> nonKeyspaceNamesList = new ArrayList<>(SchemaManager.instance.getNonSystemKeyspaces().names());
        return Collections.unmodifiableList(nonKeyspaceNamesList);
    }

    public List<String> getNonLocalStrategyKeyspaces()
    {
        return ImmutableList.copyOf(SchemaManager.instance.getNonLocalStrategyKeyspaces().names());
    }
