...
public abstract class AbstractReadExecutor
{
    ...
    static
    {
        MessagingService.instance().register(ReadCoordinationMetrics::registerReplicaLatency);
    }
    ...
    public static AbstractReadExecutor getReadExecutor(SinglePartitionReadCommand command, ConsistencyLevel consistencyLevel) throws UnavailableException
    {
        ...
        if (!allReplicas.contains(FBUtilities.getBroadcastAddress()))
            ReadCoordinationMetrics.misroutedRequests.inc();
        else if (!targetReplicas.contains(FBUtilities.getBroadcastAddress()))
            ReadCoordinationMetrics.preferredRemoteReplicas.inc();
