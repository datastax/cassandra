...
public class NonGroupingRangeCommandIterator extends RangeCommandIterator
{
    ...
    private SingleRangeResponse query(ReplicaPlan.ForRangeRead replicaPlan, boolean isFirst)
    {
        ...
        {
            Stage.READ.execute(new StorageProxy.LocalReadRunnable(rangeCommand, handler, requestTime));
        }
        else
        {
            for (Replica replica : replicaPlan.contacts())
            {
                ...
                Message<ReadCommand> message = command.createMessage(trackRepairData && replica.isFull(), requestTime);
