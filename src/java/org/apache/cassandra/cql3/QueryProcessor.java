...
public class QueryProcessor implements QueryHandler
{
    ...
    static UntypedResultSet executeOnce(String query, ConsistencyLevel cl, Object... values)
    {
        ...
        ResultMessage<?> result = statement.execute(queryState, makeInternalOptions(statement, values, cl), Dispatcher.RequestTime.forImmediateExecution());
        if (result instanceof ResultMessage.Rows)
            return UntypedResultSet.create(((ResultMessage.Rows)result).result);
        else
            return null;
    ...
    public boolean useNewPreparedStatementBehaviour()
    {
        ...
        synchronized (this)
        {
            ...
            if (minVersion.compareTo(NEW_PREPARED_STATEMENT_BEHAVIOUR_SINCE_40, true) >= 0)
            {
    ...
    throws InvalidRequestException
    {
        ...
        if (precomputedCacheEntrySize > capacityToBytes(DatabaseDescriptor.getPreparedStatementsCacheSizeMiB()))
