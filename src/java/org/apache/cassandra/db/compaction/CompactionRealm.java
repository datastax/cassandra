...
/**
 * An interface for supplying the CFS data relevant to compaction. This is implemented by {@link org.apache.cassandra.db.ColumnFamilyStore} and
...
public interface CompactionRealm extends SSTableReader.Owner
{
     ...
     */
    List<File> getDirectoriesForFiles(Set<SSTableReader> sstables);
     ...
     */
    TableSnapshot snapshotWithoutMemtable(String snapshotId);

    /**
     * Create a CFS snapshot with the given name and timestamp.
     */
    TableSnapshot snapshotWithoutMemtable(String snapshotName, Instant creationTime);

    /**
     * Change the repaired status of a set of sstables, usually to reflect a completed repair operation.
     */
    int mutateRepairedWithLock(Collection<SSTableReader> originals, long repairedAt, TimeUUID pendingRepair, boolean isTransient) throws IOException;
    ...
    boolean shouldIgnoreGcGraceForKey(DecoratedKey dk);
