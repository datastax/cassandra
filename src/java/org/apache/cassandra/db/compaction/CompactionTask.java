...
public class CompactionTask extends AbstractCompactionTask
{
    ...
    private CompactionOperation createCompactionOperation(CompactionController controller, CompactionStrategy strategy)
    {
        ...
        if (shouldReduceScopeForSpace() && !buildCompactionCandidatesForAvailableDiskSpace(actuallyCompact, transaction.opId(), !fullyExpiredSSTables.isEmpty()))
        {
    ...
    public abstract class CompactionOperation implements AutoCloseable, CompactionProgress
    {
        ...
        protected final int fullyExpiredSSTablesCount;
     ...
     */
    protected boolean buildCompactionCandidatesForAvailableDiskSpace(Set<SSTableReader> nonExpiredSSTables, TimeUUID taskId, boolean containsExpired)
    {
        ...
        while(!nonExpiredSSTables.isEmpty())
        {
            ...
            try
            {
                // TODO verify this, I'm not sure if this is ok
                ...
                List<File> newCompactionDatadirs = realm.getDirectoriesForFiles(nonExpiredSSTables);
    ...
    private void maybeNotifyIndexersAboutRowsInFullyExpiredSSTables(Set<CompactionSSTable> fullyExpiredSSTables)
    {
        if (fullyExpiredSSTables.isEmpty())
            return;

        List<Index> indexes = new ArrayList<>();
        for (Index index : realm.getIndexManager().listIndexes())
        {
        ...
        for (CompactionSSTable expiredSSTable : fullyExpiredSSTables)
        {
            if (expiredSSTable instanceof SSTableReader)
            {
                try (ISSTableScanner scanner = ((SSTableReader)expiredSSTable).getScanner())
                {
                    while (scanner.hasNext())
                    {
                        try (UnfilteredRowIterator partition = scanner.next();
                             WriteContext ctx = Keyspace.open(realm.metadata().keyspace).getWriteHandler().createContextForIndexing())
                        {
