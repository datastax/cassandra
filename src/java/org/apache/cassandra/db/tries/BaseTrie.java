...
public interface BaseTrie<T, C extends BaseTrie<T, C>>
{
    ...
    /// Call the given consumer on all (path, content) pairs with non-null content in the trie in order.
    void forEachEntry(Direction direction, BiConsumer<ByteComparable, T> consumer);
    ...
    void forEachEntrySkippingBranches(Direction direction, BiConsumer<ByteComparable, T> consumer);
