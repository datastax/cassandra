...
class SlicedCursor<T> implements Cursor<T>
{
    ...
    public Cursor<T> tailCursor(Direction dir)
    {
        ...
        switch (state)
        {
            case INSIDE:
                return sourceTail;
            case COMMON_PREFIX:
                return maybeSwapBoundsAndMakeCursor(sourceTail, duplicatableStart(), startNext, duplicatableEnd(), endNext, direction);
            case START_PREFIX:
                return maybeSwapBoundsAndMakeCursor(sourceTail, duplicatableStart(), startNext, null, -1, direction);
            case END_PREFIX:
                return maybeSwapBoundsAndMakeCursor(sourceTail, null, -1, duplicatableEnd(), endNext, direction);
            default:
                throw new UnsupportedOperationException("tailCursor on a slice boundary");
        }
    }
    ...
    private static <T> Cursor<T> maybeSwapBoundsAndMakeCursor(Cursor<T> source,
    ...
    public static <T> Cursor<T> create(Cursor<T> sourceCursor,
                                       ByteComparable left, boolean includeLeft,
                                       ByteComparable right, boolean includeRight)
    {
        Direction direction = sourceCursor.direction();
    ...
    static <T> String sliceString(ByteComparable.Version version, ByteComparable left, boolean includeLeft, ByteComparable right, boolean includeRight)
    {
