...
interface Cursor<T>
{
    ...
    /// Returns the incoming transition for this encoded position, a number between 0 and 255.
    ...
    /// Returns the incoming transition for this encoded position, taking into account the possibility of out-of-range
    /// values generated by [#positionForSkippingBranch] for use within [#skipTo] implementations.
    ///
    /// The returned value is between -1 and 256 inclusive.
    static int incomingTransitionWithOverflow(long encodedPosition)
    {
        int transitionInt = (int) encodedPosition;
        transitionInt = (transitionInt >> TRANSITION_SHIFT) & 0x1FF;
        return (((int) encodedPosition >>> DIRECTION_BIT) & 1) != 0 ? 0xFF - transitionInt : transitionInt;
    }
    ...
    static String toString(long encodedPosition)
    {
        return String.format("depth %d incomingTransition %02x%s %s",
                             ...
                             incomingTransitionWithOverflow(encodedPosition),
    ...
    /// given overflowing positions (i.e. incoming transition of 256 in forward direction, or -1 in reverse), so be sure
    /// to use [Cursor#incomingTransitionWithOverflow] to retrieve the transition requested.
    ///
