...
abstract class IntersectionCursor<T, C extends Cursor<T>> implements Cursor<T>
{
    ...
    abstract static class Slice<T, C extends Cursor<T>> extends IntersectionCursor<T, C>
    {
        ...
        int matchingPosition(int depth)
        {
            ...
            if (setState.precedingIncluded(direction.opposite()) && startsInclusive ||
                setState.precedingIncluded(direction) && endsInclusive)
            {
        ...
        public T content()
        {
            switch (state)
            {
                case SET_AHEAD:
                    return source.content();
                case MATCHING:
                    // This is a prefix (boundaries we either skip or mark as SET_AHEAD). Slices don't report prefixes.
                    return null;
                default:
