...
public class PmemRowSerializer extends UnfilteredSerializer {
    ...
    public static final org.apache.cassandra.db.memtable.pmem.PmemRowSerializer serializer = new org.apache.cassandra.db.memtable.pmem.PmemRowSerializer();
    ...
    private long serializedSize(RangeTombstoneMarker marker, SerializationHeader header, long previousUnfilteredSize, int version) {
        assert !header.isForSSTable();
        return 1 // flags
                + TypeSizes.sizeofUnsignedVInt(version)
    ...
    private void readSimpleColumn(ColumnMetadata column, DataInputPlus in, SerializationHeader header, DeserializationHelper helper, Row.Builder builder, LivenessInfo rowLiveness)
        ...
        if (helper.includes(column)) {
            Cell<ByteBuffer> cell = Cell.serializer.deserialize(in, rowLiveness, column, header, helper, ByteBufferAccessor.instance);
