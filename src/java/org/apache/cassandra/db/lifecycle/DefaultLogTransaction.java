 ...
 */
final class DefaultLogTransaction extends LogTransaction
{
    private static final Logger logger = LoggerFactory.getLogger(DefaultLogTransaction.class);
    ...
    private final Ref<DefaultLogTransaction> selfRef;
                                          ...
                                          Throwable accumulate)
    {
        ...
        for (SSTableReader reader : readers)
        {
            try
            {
                obsoletions.add(new LogTransaction.Obsoletion(reader, obsoleted(reader, logRecords.get(reader), tracker)));
            }
            catch (Throwable t)
            {
    ...
    private static class SSTableTidier implements ReaderTidier
    {
        ...
        private final Object lock;
        private final Ref<DefaultLogTransaction> parentRef;
        private final UUID txnId;
        private final boolean onlineTxn;
        private final Counter totalDiskSpaceUsed;

        public SSTableTidier(SSTableReader referent, boolean wasNew, DefaultLogTransaction parent, Tracker tracker)
        {
