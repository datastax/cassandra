 ...
 */
final class DefaultLogAwareFileLister implements LogAwareFileLister
{
    private static final Logger logger = LoggerFactory.getLogger(DefaultLogAwareFileLister.class);
    ...
    void readTxnLog(LogFile txn, OnTxnErr onTxnErr)
    {
        if (!txn.verify() && onTxnErr == OnTxnErr.THROW)
            throw new DefaultLogTransaction.CorruptTransactionLogException("Some records failed verification. See earlier in log for details.", txn);
    }
