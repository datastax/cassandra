...
{
    private final static Logger logger = LoggerFactory.getLogger(SerializationHeader.class);
    ...
    public static class Component extends MetadataComponent
    {
        ...
        private static AbstractType<?> validateAndMaybeFixColumnType(String descriptor,
                                                                     TableMetadata metadata,
                                                                     ...
                                                                     boolean isPrimaryKeyColumn)
        {
            try
            {
                type.validateForColumn(columnName, isPrimaryKeyColumn, metadata.isCounter());
                return type;
            }
            catch (InvalidColumnTypeException e)
            {
                AbstractType<?> fixed = null;
                if (fixed == null)
                {
                    ...
                    logger.error("Error reading SSTable header {}, the type for column {} in {} is {}, which is " +
                                 ...
                                 descriptor, ColumnIdentifier.toCQLString(columnName), metadata, type.asCQL3Type(),
                                 e.getMessage());
                    return type;
                }
                else
                                                                           ...
                                                                           AbstractType<?> fullType)
        {
            ...
            if (pkCount == 1)
                return validateAndMaybeFixColumnType(descriptor, metadata, pkColumns.get(0).name.bytes, fullType, true);
            ...
            return CompositeType.getInstance(validateAndMaybeFixPKTypes(descriptor, metadata, pkColumns, subTypes));
        }
                                                                        ...
                                                                        List<AbstractType<?>> pkTypes)
        {
            ...
            {
                updated.add(validateAndMaybeFixColumnType(descriptor,
                                                          table,
        ...
        public SerializationHeader toHeader(String descriptor, TableMetadata metadata) throws UnknownColumnException
        {
            ...
            for (Map<ByteBuffer, AbstractType<?>> map : ImmutableList.of(staticColumns, regularColumns))
            {
                ...
                for (Map.Entry<ByteBuffer, AbstractType<?>> e : map.entrySet())
                {
                    ...
                    AbstractType<?> type = validateAndMaybeFixColumnType(descriptor, metadata, name, e.getValue(), false);
            ...
            AbstractType<?> keyType = validateAndMaybeFixPartitionKeyType(descriptor, metadata, this.keyType);
