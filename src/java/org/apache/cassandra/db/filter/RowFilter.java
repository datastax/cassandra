...
public class RowFilter
{
    ...
    public static class Builder
    {
        ...
        {
            SimpleExpression expression = new SimpleExpression(def, op, value, indexAnalyzer(def, op), queryAnalyzer(def, op));
            add(expression);
            return expression;
        }
    ...
    public static abstract class Expression
    {
        ...
        public static class Serializer
        {
            ...
            public Expression deserialize(DataInputPlus in, int version, TableMetadata metadata) throws IOException
            {
                ...
                {
                    case SIMPLE:
                        return new SimpleExpression(column, operator, ByteBufferUtil.readWithShortLength(in), indexAnalyzer, queryAnalyzer);
                    case MAP_COMPARISON:
                        ByteBuffer key = ByteBufferUtil.readWithShortLength(in);
                        ByteBuffer value = ByteBufferUtil.readWithShortLength(in);
                        return new MapComparisonExpression(column, key, operator, value, indexAnalyzer, queryAnalyzer);
                    case VECTOR_RADIUS:
