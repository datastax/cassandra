...
public abstract class DataLimits
{
    ...
    public static abstract class Counter extends StoppingTransformation<BaseRowIterator<?>>
    {
        ...
        protected int dataSize(Row row)
        {
            if (assumeLiveData)
                return row.dataSize();

            Row purged = row.purge(DeletionPurger.PURGE_ALL, nowInSec, enforceStrictLiveness);
            return purged == null ? 0 : purged.dataSize();
        }
    }
    ...
    private static class CQLLimits extends DataLimits
    {
        ...
        protected class CQLCounter extends Counter
        {
            ...
            public void applyToPartition(DecoratedKey partitionKey, Row staticRow)
            {
                ...
                staticRowBytes = hasLiveStaticRow && bytesLimit != NO_LIMIT ? dataSize(staticRow) : 0;
            }
            ...
            {
                if (isLive(row))
                    incrementRowCount(bytesLimit != NO_LIMIT ? dataSize(row) : 0);
                return row;
            }
        ...
        protected class GroupByAwareCounter extends Counter
        {
            ...
            public void applyToPartition(DecoratedKey partitionKey, Row staticRow)
            {
                ...
                else
                {
                    ...
                    hasLiveStaticRow = !staticRow.isEmpty() && isLive(staticRow);
                    staticRowBytes = hasLiveStaticRow ? dataSize(staticRow) : 0;
                }
