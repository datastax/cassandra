...
import org.apache.cassandra.db.tries.OnDiskDeletionAwareTrieWriter;
import org.apache.cassandra.db.tries.OnDiskTrieWriter;
 ...
 * and writes the update's deletion-aware trie in the on-disk trie format with {@link OnDiskDeletionAwareTrieWriter},
...
public class TriePartitionUpdateSerializer
{
                                  ...
                                  DataOutputPlus out) throws IOException
    {
        ...
        {
            OnDiskDeletionAwareTrieWriter.write(trie,
    ...
    private static OnDiskTrieWriter.DataSerializer<Object> contentSerializer(ContentManagerPojo.PojoSerializer<Object> pojo)
    {
        return new OnDiskTrieWriter.DataSerializer<Object>()
        {
            @Override
            public int serialize(DataOutputPlus out, Object value) throws IOException
            {
                ...
                return Ints.checkedCast(out.position() - start);
            }
        };
    }

    private static OnDiskTrieWriter.DataSerializer<TrieTombstoneMarker> deletionSerializer(ContentManagerPojo.PojoSerializer<Object> pojo)
    {
        return new OnDiskTrieWriter.DataSerializer<TrieTombstoneMarker>()
        {
            @Override
    ...
    /// Sizing the trie means writing it: [OnDiskTrieWriter] emits nodes as the walk ascends and the
    /// total is only known at the end, so unlike the previous format there is no cheap size to read
    /// off the structure. A caller that then serializes therefore walks twice.
    ///
    ...
    private static long serializedTrieSize(TriePartitionUpdate trieUpdate, ContentManagerPojo.PojoSerializer<Object> pojoSerializer)
    {
        try (DataOutputBuffer trieBytes = new DataOutputBuffer())
        {
            OnDiskDeletionAwareTrieWriter.write(trieUpdate.trie(),
                                                contentSerializer(pojoSerializer),
                                                deletionSerializer(pojoSerializer),
                                                trieBytes);
