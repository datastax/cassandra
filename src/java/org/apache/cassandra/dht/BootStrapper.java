...
public class BootStrapper extends ProgressEventNotifierSupport
{
     ...
     */
    public static Collection<Token> getBootstrapTokens(final TokenMetadata metadata, InetAddressAndPort address, long schemaTimeoutMillis) throws ConfigurationException
    {
        ...
        if (allocationKeyspace != null)
            return allocateTokens(metadata, address, allocationKeyspace, numTokens, schemaTimeoutMillis);

        if (allocationLocalRf != null)
            return allocateTokens(metadata, address, allocationLocalRf, numTokens, schemaTimeoutMillis);
    ...
    {
        StorageService.instance.waitForSchema(schemaTimeoutMillis);
        if (!FBUtilities.getBroadcastAddressAndPort().equals(InetAddressAndPort.getLoopbackAddress()))
            Gossiper.waitToSettle();
